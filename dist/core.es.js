...
const loadTranslations = () => {
  return Object.fromEntries(
    Object.entries(
      /* @__PURE__ */ Object.assign({ "../lang/az.json": __vite_glob_0_0, "../lang/be.json": __vite_glob_0_1, "../lang/en.json": __vite_glob_0_2, "../lang/ru.json": __vite_glob_0_3 })
    ).map(([key, value]) => {
      var _a;
...
const buildCountriesList = (lang2) => {
  const translations = loadTranslations();
  return countriesData.map((country) => {
    var _a;
    return {
      ...country,
      name: ((_a = translations[lang2]) == null ? void 0 : _a.countries[country.country_code]) || country.country_code
    };
  });
};
const applyCountryWhitelist = (countries2, onlyCountries) => {
  if (!(onlyCountries == null ? void 0 : onlyCountries.length)) return countries2;
  const allowed = new Set(onlyCountries.map((code) => code.toLowerCase()));
  return countries2.filter((country) => allowed.has(country.country_code.toLowerCase()));
};
const getCountries = (lang2 = "ru", onlyCountries) => {
  return applyCountryWhitelist(buildCountriesList(lang2), onlyCountries);
};
const getCountryByCode = (code, lang2 = "ru", onlyCountries) => {
  return getCountries(lang2, onlyCountries).find(
    (country) => country.country_code.toLowerCase() === code.toLowerCase()
  );
};
const getCountryByName = (name2, lang2 = "ru", onlyCountries) => {
  return getCountries(lang2, onlyCountries).find(
    (country) => country.country_code.toLowerCase() === name2.toLowerCase()
  );
};
const getCountryByPhoneCode = (phoneCode, lang2 = "ru", onlyCountries) => {
  return getCountries(lang2, onlyCountries).find((country) => country.phone_code === phoneCode);
};
...
const getAllCountryNames = (countryCode) => {
  ...
  const names = Object.values(allTranslations).map((lang2) => lang2.countries[countryCode]);
...
const defaultMasks = countriesMaskData;
const getMaskForCountry = (country, masks = defaultMasks) => {
  let matrix = "###############";
  if (country) {
...
const formatPhoneInput = (value, country, masks = defaultMasks) => {
...
const filterCountries = (countries2, searchQuery, options = {}) => {
  ...
  if (!searchQuery) {
    return hideFavorites ? excludeFavorites(countries2, favoritesCountries) : countries2;
  }
  const query = searchQuery.toLowerCase();
  if (query.startsWith("+")) {
    const phoneCode = query.slice(1);
    const filtered2 = searchByPhoneCode(countries2, phoneCode);
    return hideFavorites ? excludeFavorites(filtered2, favoritesCountries) : filtered2;
  }
...
const parsePhoneNumberValue = (value, countries2) => {
  if (!value) {
